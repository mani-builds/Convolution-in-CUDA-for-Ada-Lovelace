...
__global__ void im2col_kernel(float *image,int H_in, int W_in, int im2col_H, int im2col_W, int kernel_size, float *im2col){
...
int main(){
  ...
  cudaMemcpy(kernel,kernel_h,C_in*kernel_size*kernel_size*sizeof(float), cudaMemcpyHostToDevice);
   ...
   printf("Input DIM H_in and W_in: (%d, %d)\n",H_in, W_in);
  printf("im2col DIM im2col_H and im2col_W: (%d, %d)\n",im2col_H, im2col_W);
  ...
  im2col_kernel<<<blocksPerGrid,threadsPerBlock>>>(image, H_in, W_in, im2col_H, im2col_W, kernel_size, im2col);
  ...
  for (int j = 0; j < im2col_W ; j++) {
    printf("%.1f ", im2col_h[IDX2R(i, j, im2col_W)]);
  }
  printf("\n");
  }
